...
def agent_code_analyze(original_code_snippet,error_message,edited_code_snippet):
    ...
    #     "(Remember, the original program's intent is NOT to test UB itself!)\n"
    ...
    #     "5. Using comments to resolve undefined behavior is prohibited!\n"
    ...
    #     "(IMPORTANT!!! The condition of assertions must be useful! If the condition is meaningless(like assert!(false,...) and debug_assert!(false,...)), this is incorrect.)"
    #     "\n"
...
def agent_assertion_analyze(original_code_snippet,error_message,edited_code_snippet):
    ...
    #     "   - If the assertion is unrelated or does not avoid the UB occurrence, it is incorrect.\n"
    ...
    #     "   - If the assertion changes the core program goal, this is incorrect.\n"
    ...
    #     "   - The condition of assertions must be useful! IMPORTANT! If the condition is meaningless(like assert!(false,...) and debug_assert!(false,...)), this is INCORRECT!\n"
...
def agent_fault_localize(original_code_snippet,error_message,edited_code_snippet,analysis):
    ...
    prompt = 'You will first be provided with an original code snippet in Rust language which has undefined behaviors.\n\
                ...
                For example: ```json[{"inconsistency": "<inconsistency1>", "severity": "<severity>", "inconsistency": "<inconsistency2>", "severity": "<severity>", ...}]```\
                '
    taxonomy = 'inconsistency1: Missing dependency declarations; severity: Negligible\n\
                ...
                For example: ```json[{"inconsistency": "The edited code partially change the intended functionality of the original code", "severity": "Medium"}]```\
                '
...
def extract_json_from_string(s):
    """
    从字符串中提取第一个合法的 JSON（支持 markdown 代码块包裹、数组或对象）。
    """
    # 1. 先找 markdown 代码块 ```json ... ``` 或 ``` ... ```
    code_blocks = re.findall(r"```(?:json)?\s*([\s\S]*?)\s*```", s, re.IGNORECASE)
    candidates = code_blocks if code_blocks else [s]
    for candidate in candidates:
        ...
        if match:
            try:
                ...
                continue
    raise ValueError("未能从字符串中提取有效 JSON")

def score_code(fault_localization):
    data = extract_json_from_string(fault_localization)
...
def evalutate_code(original_file_path):
    ...
    if errors != 0:
        ...
        edited_file_path = '/home/wyc/save_improvement_file/edited_code_saving/' + PATH_NAME + '_edited.rs'
