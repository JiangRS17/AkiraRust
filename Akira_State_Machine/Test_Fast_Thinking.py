    ...
    if errors != 0:        
        prompt = "You are a Rust Language Expert. Your task is to analyze the Rust code and the error message of miri test I send you, \
                 ...
                 6. Output formal: [Important] must only only only  output solutions!! must only only only  output solutions!! Do not output anything else !! And in the following format:\
                        Solution_n: [step1:xx, step2:xxx, step_n:xxx']\
                            For example:Solution_1: [step1: Refactor the unsafe allocation with a safe Rust allocator instead of raw pointers, step2: Add assertions to check if the allocation was successful, step3: Consult knowledge base for best practices in memory allocation using safe Rust]\
