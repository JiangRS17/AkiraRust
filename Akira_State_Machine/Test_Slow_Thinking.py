...
def modify_code(prompt,code_snippet,error_message):
    ...
    # print(f"Modification Suggestion from GPT-5.0:\n {suggestion}")
    # 实际修改代码逻辑（模拟）
    #initialize
...
def slow_process_code(filepath):
    ...
    if error_count != 0:
        ...
        solution_file_path = '/home/wyc/save_improvement_file/solution_saving/code_' + PATH_NAME + '_solution.txt'
...
def agent1(code,error_message):
    prompt = ("You are the Assert-Guard UB-Fix Agent. Your sole task is to insert a meaningful guarding assertion (assert! or debug_assert!) immediately to make the undefined-behaviour path become provably unreachable.\
                You will be provide with a code snippet in Rust and its error information under miri test\
                1. Read the error information from the miri test carefully and identify the undefined behavior required to handle.\
                2. Fix the code snippet. Pre-assertions are added to check for undefined behavior beforehand., thus preventing undefined behavior from occurring.\
                3. Every condition must ACTUALLY stop UB; delete pointless checks/assertions.\
                4. In particular, make sure the conditions of assertions meaningful!\
                5.  UB classes and corresponding handling methods you can refer to:\
                    A. Index-out-of-bounds - assert!(idx < len);\
                    B. Division / overflow\
                        - assert!(divisor != 0);                // for unchecked_div\
                        - assert!(a <= MAX && b <= MAX);        // for unchecked_add/mul/etc.\
                    C. Unaligned pointer - assert!(ptr as usize % mem::align_of::<T>\
                    D. Null pointer - assert!(!ptr.is_null());\
                    E. Invalid enum discriminant - assert!(discr <= MAX_VALID);\
                    F. Slice-length mismatch - assert!(src.len() == dst.len());\
                    G. Non-UTF-8 byte slice - assert!(std::str::from_utf8(bytes).is_ok());\
                    H. Allocator layout invariant - assert!(align > 1 && (align & (align - 1)) == 0, 'alignment must be a power of two and even');\
                6. Important! AVOID meaningless assertion condition like debug_assert!(false,...) and assert!(false,...))!\
                7. Should keep assertions to a minimum and make them precise!\
                8. Output only recommended code, starting with ```rust and ending with ```."
            )
...
def agent2(code,error_message):
    prompt = ("You are the Semantic-Modification Agent. You will be provide with a code snippet in Rust and its error information under miri test. And the following is the idea of repair:\
                1. Read the error information from the miri test carefully and identify the undefined behavior required to handle.\
                2. The code logic itself has problems.\
                ...
                4. Must retain the functions\APIs used in the source code(like '__rust_alloc'), but use it safely and avoid undefined behavior. \
                ...
                5.  There are UB classes and corresponding handling methods you can refer to:\
                    A. Dangling reference - requires returning an owner (String, Vec, Arc) or adding a check to determine whether it is a dangling pointer\
                    B. Stacked-Borrows / Tree-Borrows violations - fixed by Clone\
                    C. ABI mismatch - requires switching to extern-safe types\
                    D. Unwinding across FFI - requires catch_unwind\
                    E. Type-confusion via transmute - requires new enum + TryFrom or #[repr(C)] struct\
                    F.__rust_alloc - requires to state __rust_dealloc at the same time\
                    G. 'Clone' and 'Lifetime' are effective methods to handle breaking borrow rules!\
                6. Using comments to resolve undefined behavior is prohibited!\
                7. Should keep modifications to a minimum and make them precise!\
                8. Output only recommended code, starting with ```rust and ending with ```."
            )
    codepath = modify_code(prompt,code,error_message)
    code = read_code_from_file(codepath)
...
def agent3(code,error_message):
    prompt = ("You are the UB Safe-replacement Agent. You will be provide with a code snippet in Rust and its error information under miri test. And the following is the idea of repair:\
                1. Read the error information from the miri test carefully and identify the undefined behavior required to handle.\
                2. The following code is a Rust code snippet that may utilise unsafe APIs or unsafe functions. \
                3. Must retain the functions\APIs used in the source code(like '__rust_alloc'), but use it safely and avoid undefined behavior. \
                4. Must and importantly maintain the original functionality! Functionally equivalent replacement!!\
                5. There are UB classes and corresponding safe alternatives you can refer to:\
                    A. Use-after-free - fixed by Clone / Arc / Rc or delete null pointer use\
                    B. Out-of-bounds indexing - fixed by get/get_mut\
                    C. Unaligned read/write - fixed by read_unaligned/write_unaligned\
                    D. Data races - fixed by Atomic* or Mutex/RwLock\
                    E. Null/mis-aligned ref - fixed by NonNull::new + as_ref\
                    F. Mutation through shared ref - fixed by Cell/RefCell/Atomic*\
                    G. Invalid bool/enum/bit-pattern - fixed by validated transmute or bytemuck::checked::cast\
                    H. Improper MaybeUninit::assume_init - fixed by write-then-assume_init\
                    I. __rust_alloc - requires to state __rust_dealloc at the same time\
                    J. 'Clone' and 'Lifetime' are effective methods to handle breaking borrow rules!\
                6. Using comments to resolve undefined behavior is prohibited!\
                7. Should keep replacements to a minimum and make them precise!\
                8. Output only recommended code, starting with ```rust and ending with ```."
            )
    codepath = modify_code(prompt,code,error_message)
    code = read_code_from_file(codepath)
...
def knowledge_modification(result,code,error_message):
    ...
    prompt = f"1, Q:{sample_code}.\
              ...
              6. Should keep repair operations to a minimum and make them precise!\
              6. Output only recommended code, starting with ```rust and ending with ```..\
