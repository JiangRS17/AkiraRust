...
def count_rust_files(folder_path):
    #count the amount of rust files in a folder
    count = 0
    for file in os.listdir(folder_path):
        if os.path.isfile(os.path.join(folder_path, file)) and file.endswith('.rs'):
            count += 1
    return count
...
def evalutate_code(original_file_path):
    ...
    if errors != 0:
        ...
        edited_folder_path = '/home/wyc/save_defect_curve/edited_code_saving/' + PATH_NAME

        if os.path.isdir(edited_folder_path): #judge the edited folder existed or not
            #solutions CAN solve the undefined behavior
            edited_code_num = count_rust_files(edited_folder_path) #calculate the number of edited codes
            edited_code_evaluated_num = 1

            while edited_code_evaluated_num <= edited_code_num: #score the edited code one by one
                edited_file_path = edited_folder_path + '/' + PATH_NAME + '_edited' + str(edited_code_evaluated_num) + '.rs'
                ...
                print(f"{PATH_NAME}_{edited_code_evaluated_num} Evaluation:\n{fault_localization}")
                print(f"{PATH_NAME}_{edited_code_evaluated_num} Score: {score}")

                # 记录语义评估分数信息，将分数信息收集到相关文件夹中
                record_folder_path = '/home/wyc/save_defect_curve/score_recording/' + PATH_NAME
                os.makedirs(record_folder_path, exist_ok=True)
                record_file_path = record_folder_path + '/' + PATH_NAME + '_record' + str(edited_code_evaluated_num) + '.txt'
                with open(record_file_path, 'w', encoding='utf-8') as file:
                    file.write(f"Analysis:\n{analysis}\n\nFault localization:\n{fault_localization}\n***Score:***{score}")

                # === 新增：在 defect_record.txt 中插入 Semantic Score ===
                defect_record_path = f"/home/wyc/save_defect_curve/defect_recording/{PATH_NAME}_defect_record.txt"
                if os.path.exists(defect_record_path):
                    with open(defect_record_path, "r", encoding="utf-8") as f:
                        lines = f.readlines()
                    # 找到第 edited_code_evaluated_num 个以 Success! 结尾的行
                    success_count = 0
                    insert_idx = None
                    for idx, line in enumerate(lines):
                        if line.rstrip().endswith("Success!"):
                            success_count += 1
                            if success_count == edited_code_evaluated_num:
                                insert_idx = idx + 1
                                break
                    if insert_idx is not None:
                        # 检查下一行是否已是 Semantic Score
                        if insert_idx >= len(lines) or not lines[insert_idx].startswith("Semantic Score:"):
                            lines.insert(insert_idx, f"Semantic Score: {score}\n")
                            with open(defect_record_path, "w", encoding="utf-8") as f:
                                f.writelines(lines)
                        else:
                            print("Semantic Score Existed!")
                            pass
                    else:
                        print(f"Warning: 未找到第{edited_code_evaluated_num}个Success!/Succsess!，未插入Semantic Score")
                else:
                    print(f"Warning: {defect_record_path} 不存在, 未插入Semantic Score")
            ...
            record_folder_path = '/home/wyc/save_defect_curve/score_recording/' + PATH_NAME + '_failed'
            os.makedirs(record_folder_path, exist_ok=True)
            record_file_path = record_folder_path + '/' + PATH_NAME + '_record.txt'
            with open(record_file_path, 'w', encoding='utf-8') as file:
