...
def save_new_code(name,code,passed):  #True or False
    directory = "/home/wyc/save_defect_curve/pass" if passed else "/home/wyc/save_defect_curve/failure"
...
def slow_process_code(filepath):
    code = read_code_from_file(filepath) # 此时code里面的内容为原始代码
    global Best_Code
    Best_Code = code
    output, returncode = run_cargo_miri(code)
    error_count = error_calculate(output) # error_count记录未定义行为的数量
    print("Begain: ",error_count," errors left")
    original_error_count = error_count # original_error_count记录初始的未定义行为的数量
   
    if error_count != 0:
        solution_file_path = '/home/wyc/save_defect_curve/solution_saving/code_' + PATH_NAME + '_solution.txt'
        ...
        # successful_count用于记录一套10个solution的方案中有多少条solution成功解决了未定义行为
        successful_count = 0

        # 用于记录缺陷曲线数据的txt文件,一个缺陷rust代码对应一个记录文件
        defect_record_path = '/home/wyc/save_defect_curve/defect_recording/'+ PATH_NAME + '_defect_record.txt'
        
        # 首先利用refine函数记录处理的原始代码的特征
        print("Generating Code Features...")
        original_code_tag = refine(code,output)
        with open(defect_record_path, 'w') as file:
            file.write(f"Code Features: {original_code_tag}\n")

        # 正式开始执行10个solution的方案
        for line in content:
            error_count = original_error_count # 执行每一条solution之前，error_count必须归位为初始error数量
            ...
            if match:
                ...
                # 在具体执行一条solution之前，需要记录原始代码，并且将最佳代码初始化为原始代码
                original_code = read_code_from_file(filepath) # original_code里记录了原始代码的内容
                Best_Code = original_code

                print(f"Processing Solution {solution_num}...")
                # solution_num_record用于记录当前执行到第几条solution了
                solution_num_record = f"Solution {solution_num}"
                with open(defect_record_path, 'a') as file:
                    file.write(f"{solution_num_record}\n")
                
                max_rounds = 2  # 最大迭代轮次
                rounds = 0 # 当前迭代轮次
                error_count_past = error_count
                called_agents = set() # 用于记录使用过的agents
                while error_count != 0 and rounds < max_rounds:
                    count_step = 0 # count_step用于记录执行到第几步，且应该在一个轮次内进行记录
                    rounds += 1
                    # solution_excuting_agents记录这一轮次执行了哪些agents，miri_errors_count记录了这一轮次的error数量变化
                    solution_excuting_agents = f"Agents Excuted ===Round{rounds}=== :"
                    miri_errors_count = f"Miri Errors ===Round{rounds}=== : Begain with {original_error_count}errors "
                    code = original_code # 在进行agent处理之前，code的内容需要归位为原始代码
                    print(f"=== Round {rounds} ===")
                    for step in steps: # 执行一条solution中的agent
                        agent_match = re.search(r"Agent(\d+)", step)
                        if agent_match:
                            count_step+=1
                            agent_key = agent_match.group(1)
                            called_agents.add(agent_key) # 记录已经使用了哪些agents
                            ...
                            if error_count != 0:
                                solution_excuting_agents = solution_excuting_agents + f" Agent{agent_key}"
                                miri_errors_count = miri_errors_count + f" {error_count}errors"
                            if error_count == 0:
                                solution_excuting_agents = solution_excuting_agents + f" Agent{agent_key} Solved!"
                                miri_errors_count = miri_errors_count + f" {error_count}errors Success!"
                                # effective_step_num = count_step
                                successful_count += 1
                                break
                    # 当steps全部执行完成，但是仍然为解决未定义行为：补充未调用的agent
                    if error_count != 0:
                        all_agents = {'1', '2', '3', '4', '5'}
                        missing_agents = all_agents - called_agents
                        # 在执行补充agents之前，做好补充标识，supplement后为补充执行的agents
                        solution_excuting_agents = solution_excuting_agents + " Supplement:"
                        miri_errors_count = miri_errors_count + " Supplement:"
                        for agent_key in missing_agents:
                            count_step += 1
                            print(f"Step{count_step}: ","Excuting Supplement Agent",agent_key)
                            code = agent_function_call(agent_key,code,output)
                            error_count_past = error_count
                            output, returncode = run_cargo_miri(code)
                            error_count = error_calculate(output)
                    ...
                    # 记录一轮次执行后修复代码的操作应在轮次循环内完成
                    # 先记录一下缺陷曲线数据
                    with open(defect_record_path, 'a') as file:
                        print("Taking down defect curve...")
                        file.write(f"{solution_excuting_agents}\n")
                        file.write(f"{miri_errors_count}\n")

                # 执行完了solution的所有轮次
                if error_count == 0:
                    print(f"{code}\n")
                    edited_code_folder = '/home/wyc/save_defect_curve/edited_code_saving/' + PATH_NAME
                    os.makedirs(edited_code_folder, exist_ok=True)
                    edited_code_file = edited_code_folder + '/' + PATH_NAME + '_edited'+ str(successful_count) + '.rs'
                    with open(edited_code_file, 'w') as file:
                        file.write(code)
                    
                    #save the passed solution(记录缺陷曲线不需要这个模块)
                    # passed_solution_saving_file = '/home/wyc/save_defect_curve/solution_pass_filter/' + PATH_NAME + '_passed_solution.txt'
                    # with open(passed_solution_saving_file, 'a') as file:
                    #     effective_step_count = 0
                    #     effective_solution = f'Solution_{solution_num}: ['
                    #     #only remain the effective executed steps
                    #     for step in steps:
                    #         effective_step_count+=1
                    #         if effective_step_count < effective_step_num:
                    #             effective_solution = effective_solution + step + ', '
                    #         if effective_step_count == effective_step_num:
                    #             effective_solution = effective_solution + step + ']'
                    #     print('Effective executed steps: ',effective_solution)
                    #     file.write(f"{effective_solution}\n") 
            
            # 不需要在miri报错为0时跳出10条solution方案执行列表
        
        # 执行完了所有solutions，如果没有一条solution能成功修复error
        if successful_count == 0:
            print(f"No solution can solve the error of {PATH_NAME}!")
            fail_code_file = '/home/wyc/save_defect_curve/failure/' + PATH_NAME + '_failed.rs'
